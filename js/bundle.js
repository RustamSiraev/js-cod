...
function slider({container, slide, nextArrow, prevArrow, totalCounter, currentCounter, wrapper, field, stepInterval = 5000}) {
    ...
    let timer = 0
    makeTimer(stepInterval)
    function makeTimer(stepInterval){
        clearInterval(timer)
...
function timer(id, deadline) {
    ...
    let promotionEnd = document.querySelector('.promotion_end'),
        endDate = new Date(Date.parse(deadline)),
        options = {month: 'long', day: 'numeric'}

    promotionEnd.innerHTML = (endDate.toLocaleString("ru", options))
